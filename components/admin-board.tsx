  ...
  return (
    <section aria-label="Administración" className="mx-auto max-w-6xl">
      <motion.div variants={withReducedMotion(pageVariants, reduced)} initial="initial" animate="animate" className="mb-7">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--muted)]">Organizá las sesiones compartidas y mantené actualizadas sus asignaciones.</p>
      </motion.div>
      <AnimatePresence>
        {error ? (
          <motion.p key="admin-error" initial={reduced ? { opacity: 0 } : { opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.18 }} role="alert" className="mb-5 rounded-xl bg-rose-500/10 px-4 py-3 text-sm text-rose-600">
            {error}
          </motion.p>
        ) : null}
      </AnimatePresence>
      <AnimatePresence>
        {success ? (
          <motion.p key="admin-success" initial={reduced ? { opacity: 0 } : { opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.18 }} role="status" className="mb-5 rounded-xl bg-emerald-500/10 px-4 py-3 text-sm text-emerald-700">
            {success}
          </motion.p>
        ) : null}
      </AnimatePresence>
      ...
      <motion.div variants={withReducedMotion(pageVariants, reduced)} initial="initial" animate="animate" className="rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-4 shadow-[0_12px_30px_rgb(15_23_42/0.04)] sm:p-6">
        ...
        <motion.div variants={withReducedMotion(staggerContainer, reduced)} initial="hidden" animate="visible" className="space-y-3">
          {subjects.map((subjectItem) => { const subjectSessions = sessions.filter((item) => item.subject_id === subjectItem.id); if (subjectSessions.length === 0) return null; return (
            <motion.div key={subjectItem.id} variants={withReducedMotion(staggerItem, reduced)} className="rounded-xl bg-[var(--background)] p-3 sm:p-4">
              <h4 className="text-sm font-semibold text-[var(--ink)]">{subjectItem.code} · {subjectItem.name}</h4>
