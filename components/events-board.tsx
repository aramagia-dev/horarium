    ...
    <section aria-label="Eventos" className="mx-auto w-full max-w-5xl min-w-0 max-w-full overflow-x-hidden">
      ...
      <AnimatePresence mode="wait">
        {visibleEvents.length === 0 ? (
          ...
          <motion.div
            key={`${typeFilter}-${statusFilter}-${subjectFilter}`}
                  ...
                  <div className="flex min-w-0 gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h2 className="text-base font-semibold text-[var(--ink)]">{event.title}</h2>
                        <span className="rounded-full bg-[var(--soft)] px-2 py-1 text-[10px] font-bold text-[var(--accent)]">{labels[event.type]}</span>
                      </div>
                      <p className="mt-1 text-sm text-[var(--muted)]">{event.time ? `${event.time.slice(0, 5)} · ` : "Todo el día · "}{event.subject_code ?? "Sin materia"} · {labels[event.status]}{isEventOverdue(event) ? " · Vencido" : ""}</p>
                      {event.description ? <p className="mt-2 text-sm leading-6 text-[var(--foreground)]">{event.description}</p> : null}
                    </div>
                  </div>
                  {canEdit(event) ? (
                    <div className="flex shrink-0 gap-3">
                      <button type="button" onClick={() => beginEdit(event)} className="text-xs font-semibold text-[var(--accent)] hover:underline">Editar</button>
