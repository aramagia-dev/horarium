...
export function NotesBoard({ schedule }: { schedule: ScheduleEntry[] }) {
  ...
  return <section aria-label="Notas" className="mx-auto w-full max-w-[1440px]">
    <div className="mb-7 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      <label className="block w-full lg:ml-auto lg:w-[360px] lg:max-w-[360px] shrink-0 lg:mr-16 xl:mr-16"><span className="sr-only">Buscar materias</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por materia, código o sección..." aria-label="Buscar materias" className="w-full rounded-xl border border-[var(--line)] bg-[var(--surface)] px-4 py-3 text-sm text-[var(--ink)] outline-none placeholder:text-[var(--muted)] focus:border-[var(--accent)]" /></label>
    </div>
