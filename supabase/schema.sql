...
create policy "Admins delete academic events" on public.academic_events for delete to authenticated using (exists (select 1 from public.profiles where id = auth.uid() and role = 'admin'));
